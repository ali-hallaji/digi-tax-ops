...
| S2-07 | پیش‌دریافت/پیش‌پرداخت (A7) | Staff+ | «قبل از فاکتور پول گرفتم/دادم» | PAY → «ثبت تراکنش جدید» → تیک «این یک پیش‌دریافت است» (سمت خرید: «پیش‌پرداخت») | `is_advance` payment: party+account required, no doc link; excluded from FIFO and party balances until applied; `POST /payments/{id}/apply-advance` links to exactly ONE doc (amount ≤ remaining) and stamps `applied_at` | ردیف با برچسب «پیش‌دریافت»؛ خط جداگانه روی کارت طرف + ستون گزارش؛ بنر یک‌کلیکی «استفاده شود؟» در پنجرهٔ تسویه | supersedes the S2-05 unallocated-payment model — advances are explicit, never silently netted; merchant copy only «پیش‌دریافت/پیش‌پرداخت», never «علی‌الحساب» |
...
| S3-10 | خرج چک دریافتی برای خرید (A6) | Staff+ | «چک مشتری را به تأمین‌کننده می‌دهم» | پنجرهٔ پرداختِ خرید → «روش‌های بیشتر» → «استفاده از چک دریافتی (خرج چک)» → برگهٔ تأیید → «چک را خرج کن» | `POST /cheques/{id}/spend` {purchase_id}: only received+in_progress, amount ≤ purchase remaining; TWO payment rows (both cheque_id, account_id NULL — CHECK `ck_payments_account_or_cheque`), atomic dual recompute; NO treasury movement | چک «خرج‌شده» (پیل خاکستری آرام + spent_at)؛ فاکتور مشتری و خرید هر دو تسویه | bounce-after-spend deferred to Batch B (guide pitfall says contact support); no vendor-credit modeling — oversized cheques simply not offered |
...
| S8-08 | تغییر رمز عبور | Any | «رمزم را عوض کنم» | SET → «حساب من» → «رمز عبور» → «تغییر رمز عبور» (built, Phase-2) | current-password check; ≥8 chars upper/lower/digit; never stored raw | رمز جدید از ورود بعدی | guide walkthrough S8-08 (content.ts) |
| S8-09 | تغییر پوسته (روشن/تاریک) | Any | «تم تاریک می‌خواهم» | SET → «حساب من» → «پوسته برنامه» → «روشن»/«تاریک» (built) | per-user pref persisted; pre-paint theme script | فوری + ورودهای بعدی | guide walkthrough S8-09 |
| S8-10 | واحد نمایش ریال/تومان (راهنما) | Owner/Admin | «تومانی ببینم» | SET → «تنظیمات کسب‌وکار» → «واحد نمایش مبالغ» | same as S8-05 (display-only) | همه صفحات فوراً | guide walkthrough S8-10 documents the built S8-05 control |
| S8-11 | شروع سال مالی | Owner/Admin | «سال مالی من فروردین نیست» | SET → «شروع سال مالی» → ماه/روز → «ذخیره تغییرات» (built, Phase-2 D6) | fiscal-year anchor for report windows; past-year reports unaffected | بازه گزارش‌ها بر اساس روز انتخابی | guide walkthrough S8-11 (with «تأثیر نمی‌گذارد» note) |
...
**Product frame (locked):** every scenario here is behind the per-business
«نمای حسابدار» toggle (default OFF). With the toggle OFF none of these surfaces exist and
every merchant screen is pixel-identical to today. Inside this layer, real accounting
vocabulary (سند/بدهکار/بستانکار/معین/تراز/دفتر) is CORRECT and required — this is the one
place the jargon ban is lifted. The journal is a deterministic, idempotent **replay** of
the same complete events phase 1 already records (S7-06) — no merchant write path is
coupled to it.

| ID | سناریو | Actor | Trigger | Flow → primary action | پشت صحنه | End-state | Duplicate-risk |
|---|---|---|---|---|---|---|---|
| S9-01 | روشن‌کردن نمای حسابدار | Owner/Admin | «حسابدار دارم / سند و تراز می‌خواهم» | SET → «تنظیمات کسب‌وکار» → کارت «نمای حسابدار» → کلید روشن | `accountant_view_enabled=true`; turning ON triggers a per-tenant journal regenerate | گروه «نمای حسابدار» با ۴ صفحه در سایدبار ظاهر می‌شود؛ صفحه‌های همیشگی بدون تغییر | guide S9-01; toggle owner-gated + server toggle-gated (403 friendly) |
| S9-02 | دیدن سند یک تراکنش | Owner/Admin | «این فاکتور/پرداخت پشتش چه سندی است؟» | فاکتور/پرداخت/چک/هزینه → لینک «دیدن سند» | `GET /accounting/entry-for` → ناوبری به صفحهٔ کامل سند `/app/accounting/entries/:id` (P4: قابل چاپ + «اصلاح تراکنش مبدأ») | صفحهٔ کلاسیک سند همان تراکنش | guide S9-02; link renders ONLY when toggle ON (null otherwise) |
| S9-03 | خواندن دفتر حساب | Owner/Admin | «گردش و ماندهٔ صندوق/بانک را ببینم» | «نمای حسابدار» → «دفتر حساب‌ها» → انتخاب حساب | `GET /accounting/ledger` running balance | گردش کامل + ماندهٔ در حال اجرا | guide S9-03; ماندهٔ پایان == گزارش‌های ساده |
| S9-04 | گرفتن تراز آزمایشی | Owner/Admin | «دفترها سالم است؟» | «نمای حسابدار» → «تراز آزمایشی» → انتخاب‌گر «دورهٔ مالی» + دکمه‌های ۲/۴/۶/۸ ستونی (P4) | `GET /accounting/trial-balance?columns=`; pairs اول دوره/گردش/جمع کل/مانده + balanced flag per pair | نوار سبز «دفترها تراز است» وقتی همهٔ جفت‌ستون‌ها برابرند | guide S9-04 |
| S9-05 | خروجی CSV/Excel برای حسابدار | Owner/Admin | «به حسابدارم بدهم» | دفتر روزنامه/دفتر حساب‌ها/تراز آزمایشی → «خروجی CSV» یا «خروجی Excel» (P4) | CSV با BOM/فارسی؛ xlsx واقعی RTL با سرستون فریز و جداکنندهٔ سه‌رقمی | فایل CSV/xlsx دانلود می‌شود | guide S9-05; بدون واگذاری دسترسی کامل |
| S9-06 | مرور و ویرایش درخت حساب‌ها | Owner/Admin | «ساختار حساب‌هایم چیست؟ / زیرحساب می‌خواهم» | «نمای حسابدار» → «درخت حساب‌ها» → منوی هر نود (P4: افزودن زیرحساب/تغییر نام/بایگانی) | `GET /accounting/chart` + `POST/PATCH /accounting/chart/accounts` + archive/unarchive؛ کد خودکار؛ گروه/کل قفل | درخت گروه←کل←معین←تفصیلی؛ بایگانی به‌جای حذف | guide S9-06; تفصیلی‌های سیستمی خودکار ساخته می‌شوند |
...
| Journal/debit/credit | everywhere | never rendered on merchant screens; events recorded silently (S7-06). **Batch B exception:** rendered CORRECTLY inside the opt-in «نمای حسابدار» layer only (Group 9) — invisible until the owner turns the toggle on |
