...
| S9-02 | دیدن سند یک تراکنش | Owner/Admin | «این فاکتور/پرداخت پشتش چه سندی است؟» | فاکتور/پرداخت/چک/هزینه → لینک «دیدن سند» | `GET /accounting/entry-for` → سند دوطرفه در پنجرهٔ کوچک | سند بدهکار/بستانکار همان تراکنش | guide S9-02; link renders ONLY when toggle ON (null otherwise) |
