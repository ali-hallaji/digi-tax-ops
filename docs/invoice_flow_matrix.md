...
- **Corrective «خارج از الگو» تذکر — CLOSED, sandbox-proven (2026-07-26).** Two otherwise-identical correctives on the نیک‌تجارت sandbox differed ONLY in `inp`/`inty`: present → `"warning": [14007, 14004]`; omitted → `"warning": []`. An اصلاحیه/ابطال/برگشت refers to a registered invoice, so the org reads الگو and نوع from the REFERENCE and repeating them is «خارج از الگو». Referring subjects (ins 2/3/4) now omit both from the emitted header (`_drop_pattern_fields_on_referring_subject`); the mapper still uses them to CHOOSE the pattern, so unsupported combinations are still refused. Full write-up: `docs/moadian/corrective_inp_inty_experiment_2026-07.md`. The remaining referring-subject question (which OTHER fields to blank beyond the buyer) stays with the accountant.
