...
- **E2E harness shipped + stabilized (2026-06-18–19):** Playwright 6-spec harness in `digi-tax-frontend/e2e/`. `pnpm e2e` (headless) · `pnpm e2e:watch` (headed, 1 worker, slowMo 800ms, Persian caption card per spec) · `pnpm e2e:headed` (headed all workers). All 6 specs green; spec 04 confirmed non-flaky 10/10. White-screen flash fixed: stage_0 redirect moved to `beforeLoad` so no empty body renders post-login. Watch mode captions: dark-themed Persian card injected before each spec showing scenario title + expected outcome; zero cost in headless (WATCH_MODE env gate).
