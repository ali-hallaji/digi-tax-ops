...
## 2026-08-07 (۲) — بلاکر پیامک بسته شد + تماس لندینگ + حادثهٔ پرشدن دیسک dev

SHAs: backend **`e31e24a`** · frontend **`bb1c834`** · ops **`181c0ff`**.
بدون مهاجرت تازه (head `inv1405a001`).

### پیامک — علت ما بود، و برطرف شد
`SMS_ALLOWLIST` روی dev به `09120000000,09395508721` رسید و شمارهٔ واقعیِ مؤسس
مجاز شد (تأییدشده روی خودِ سرور). **یک** OTP واقعی فرستاده شد (بدون طوفان):
messageid **۱۸۲۷۲۶۴۱۲**. وضعیت کاوه‌نگار: **۵ «ارسال به مخابرات»** و روی همین
وضعیت ماند (۵۰ دقیقه پایش). این **قطعاً با گذشته فرق دارد**: ارسال‌های قبلی به
شمارهٔ ساختگی وضعیت **۱۱ «نرسیده به گیرنده»** می‌گرفتند. یعنی پیام حالا به
مخابرات می‌رسد؛ رسید تحویل (DLR) برنگشته، که روی خطوط خدماتی ایران عادی است.
**تأیید نهایی با خودِ مؤسس است — گوشی دستش بود.**
ستون «تحویل به گیرنده» در پنل ادمین همین را زنده نشان می‌دهد (`[pending]`)، نه
«رسیده» — یک تشخیص هرگز نباید دروغِ خوش‌بینانه بگوید.

### 🔥 حادثه: دیسک dev صددرصد پر شد (و هارنس را قرمز کرد)
چند `build --no-cache` پشت‌سرهم، ۵۲ ایمیج / ۲۴٫۱ گیگ لایهٔ بی‌استفاده جا گذاشت.
دیسک ۳۸ گیگی پر شد و پستگرس در حلقهٔ کرشِ checkpoint افتاد
(`No space left on device`). **نشانه گمراه‌کننده بود:** سایت ۲۰۰ می‌داد و
کانتینرها Up بودند، ولی کلِ هارنس تایم‌اوت می‌شد.
درمان: `docker image prune -af --filter 'until=6h'` ⇒ **۲۰٫۸۸ گیگ آزاد**، پستگرس
سالم برگشت، و **هیچ داده‌ای از دست نرفت** (۱۵ مستأجر، ۱۰۷۲ فاکتور، ۱۸۱۵ سند).
محافظ دائمی: prune به `prod_bring_up.sh` اضافه شد و قاعده‌اش در runbook ثبت.
**اگر هارنس یکجا و گسترده قرمز شد، اول `df -h /`.**

### لندینگ — تماس واقعی
تلفن ۰۹۳۹۵۵۰۸۷۲۱ با `tel:` در فوتر و صفحهٔ تماس؛ تلگرام و بله فقط با آیکون
(شناسه‌ها هرگز چاپ نمی‌شوند). ردیف ایمیل حذف شد. نشان اینماد دست‌نخورده.

### قوانین دائمی تازه (workspace CLAUDE.md §2)
«مرجعِ مصرف‌شدنی» و «یک لندینگ، یک کدبیس» (بستهٔ استاتیک apex بازنشسته).


## 2026-08-07 — چرا پیامک نمی‌رسید (بستهٔ راه‌انداز) + برگشت سند حقوق + پای برگشت انبار

SHAs: backend **`e31e24a`** · frontend **`93fe62d`** · ops **`0b43288`**.
بدون مهاجرت تازه (head همچنان `inv1405a001`).

### 🔴 ریشهٔ «پیامک نمی‌رسد» — تشخیص تجربی، نه حدس

**در یک جمله:** روی dev فقط `SMS_ALLOWLIST=09120000000` مجاز است — یعنی یک شمارهٔ
**ساختگیِ seed** که هیچ گوشی‌ای پشتش نیست — پس هر شمارهٔ واقعی پیش از رسیدن به
کاوه‌نگار `suppressed` می‌شود و تنها شماره‌ای که واقعاً برایش می‌فرستیم، از
مخابرات جواب «نرسیده به گیرنده» می‌گیرد.

شواهد (همه از API خودِ کاوه‌نگار، با کلیدِ خوانده‌شده از `.env`؛ کلید هیچ‌جا چاپ
یا ذخیره نشد):

| بررسی | نتیجه |
|---|---|
| `account/info` | ✅ سالم — اعتبار ۷٬۰۶۱٬۵۲۱، نوع Master |
| `account/config` | ✅ `debugmode=disabled` (ارسال واقعی)، خط پیش‌فرض `10006000900` |
| قالب `digiotp` | ✅ پذیرفته — ارسال‌ها messageid برگردانده‌اند |
| `sms/status` — دو پیام اخیر (۱۶۴۳۴۷۹۹۴ / ۱۶۴۳۴۸۵۹۹) | ❌ **status=11 «نرسیده به گیرنده»** |
| `sms/status` — پنج پیام قدیمی‌تر | `status=100` «شناسه نزد اپراتور یافت نشد» (از بازهٔ نگهداری وضعیت گذشته‌اند) |
| گیرندهٔ **همهٔ** ۷ ارسال واقعی | `0912***0000` — یعنی همان شمارهٔ ساختگی |
| شماره‌های واقعی در دفتر (`0912***4552`، `0919***0856`، `0930***8159`) | همه `suppressed` — **اصلاً فرستاده نشدند** |

**پس مشکل از کاوه‌نگار نیست؛ از پیکربندی ماست.**

**کاری که مؤسس باید بکند:** شمارهٔ واقعیِ خودش را به `SMS_ALLOWLIST` روی dev
اضافه کند (کاما-جدا)، یا برای تولید فهرست را خالی بگذارد تا همه بگیرند. در پنل
کاوه‌نگار **هیچ کاری لازم نیست** — حساب، اعتبار، خط و قالب همه سالم‌اند.

**که دیگر معما نشود:** جدول «آخرین پیامک‌ها» حالا ستون **«تحویل به گیرنده»** دارد
که وضعیت واقعیِ مخابرات را نشان می‌دهد. «ارسال شد» فقط یعنی سرویس‌دهنده تحویل
گرفت — همین شکاف بود که هفته‌ها جدول را سالم نشان داد. نبودِ پاسخ «نامشخص» است،
نه «رسیده». روی dev زنده تأیید شد: `status=sent` ولی `delivery=نرسیده به گیرنده`.

### حقوق: «بازگرداندن به پیش‌نویس» برای سند پرداخت‌شده
سند پرداخت‌شده نه ویرایش می‌شد نه برگشت، و پیام امتناع راه خروج را نمی‌گفت. حالا
برگشت **رد نمی‌شود، تأیید می‌شود** (پرچم `reverse_payment`): سند پرداختِ حقوق مشتق
است، پس پاک‌شدن `status=paid` و `paid_from_account_id` آن را در بازتولید بعدی حذف
می‌کند و خزانه برمی‌گردد. اقساط وامِ همان run هم آزاد می‌شود و وضعیت وام از نو
مشتق می‌شود — همان درسِ ماندهٔ وام. پیام‌های امتناع (سند حقوق، حذف سند حقوق، حذف
پرداختِ وصل‌شده) حالا دقیقاً راه خروج را نام می‌برند.

### انبار: پای برگشت‌ها (شکاف نام‌بردهٔ بستهٔ قبل بسته شد)
جدول جهت پول برای هر چهار حالت سنجاق شد. سطر کلیدی: **برگشت از فروش با نرخِ اصلیِ
خروجِ همان فاکتور برمی‌گردد**، نه با میانگین امروز — وگرنه روی فروشی که باطل شده
سود یا زیان ساخته می‌شود. برگشت از خرید با نرخ سند خارج می‌شود.

### شایگان: نقشهٔ ستون‌ها بانک شد (`docs/shaygan_import_column_maps.md`)
هر شش فایل BIFF/OLE2 واقعی، ترتیب ستون برعکسِ RTL، بنر ۱ یا ۲ سطر (نه ثابت).
هر دو تلهٔ دستور روی خودِ فایل‌ها تأیید شد. **یافتهٔ سوم:** ستون «هزینه» در فایل
*خرید* خالی است، پس «هزینه = ارزش افزوده» قاعدهٔ ساختاری نیست و واردکننده باید
نسبت را بسنجد. خواندنِ این فایل‌ها به `xlrd` نیاز دارد که در ایمیج نیست.


## 2026-08-06 (۲) — انبار: هستهٔ بهای تمام‌شده و سند مرکب (backend فقط)

SHAs: backend **`5f33571`** · frontend **`eeda8ba`** · ops **`ea118b7`**.
مهاجرت **`inv1405a001`** — روی dev اجرا و با psql تأیید شد (head = inv1405a001).
**`inventory_enabled` روی هیچ کسب‌وکاری روشن نیست (۰ مستأجر)** ⇒ رفتار هیچ
کسب‌وکاری تغییر نکرده است.

### ✅ چه چیزی تحویل شد (با اثبات)
- **موتور میانگین موزون متحرک** — replay خالص، بدون دیتابیس. هر دو مثالِ
  سنجاق‌شده سبز: چسب (میانگین ۱۱۶۶، بهای ۱۱ واحد **۱۲٬۸۲۶**، مانده ۴) و کاردکس
  هیراد (۳ @ **۴۷٬۰۰۰٬۰۰۰**، خروج به همان نرخ). روی **ایمیجِ مستقر روی dev** هم
  اجرا و همان اعداد بازتولید شد.
- **تصمیمِ گِردکردن که مثالِ چسب حل کرد:** میانگین ROUND_DOWN می‌شود و خروج با
  همان نرخِ گردشده قیمت می‌خورد (۱۲٬۸۲۶، نه ۱۲٬۸۳۳). پیامدش — ماندهٔ چند ریال
  بالاتر روی واحدهای باقی‌مانده — پذیرفته و در `inventory_reference_notes.md`
  ثبت شد.
- **دفتر کاردکس مشتق** — مثل دفتر روزنامه هر بار از نو replay می‌شود، پس سند با
  تاریخ عقب‌تر حالت خاص نیست. موجودی منفی مجاز است و ردیف «نرخ برآوردی» می‌خورد.
- **سند مرکب** — فروش در همان سند بهای تمام‌شده بد / موجودی کالا بس می‌زند؛ خرید
  در حالت دائمی موجودی کالا بد می‌زند نه خرید. کاردکس و سند به ریال یکی‌اند
  (عدد از خودِ کاردکس خوانده می‌شود). تراز کل مستأجر سنجاق شد.
- **گاردِ رگرسیون:** کسب‌وکار بدون ماژول دقیقاً همان سه پای قبلی را می‌زند و هیچ
  ردیف کاردکسی نمی‌گیرد.
- **درسِ چارت:** دو حسابِ موجودی کالا / بهای تمام‌شده در اسکلت seed **نشدند** —
  الگوی «بازرگانی» همین دو عنوان را دارد و کدشان را پویا می‌گیرد؛ hardcode کردن
  کدِ الگو را می‌دزدید و آن‌ها را is_system می‌کرد. سه آزمونِ الگو همین را گرفتند.
  حالا `ensure_inventory_accounts` با **عنوان** get-or-create می‌کند.
- **اسپک بایگانی** دیگر به retry وابسته نیست: به‌جای متنِ کل صفحه، یک عنصر پایدار
  (`pnl-profit`) را با انتظار واقعی می‌خواند. محلی دو بار بارِ اول سبز، و در هر
  دو اجرای dev **صفر flaky**.
- **گیت:** pytest ۱۶۴۴ pass / ۵ fail (همان baseline مستند) / ۹ skip؛ ruff+black
  تمیز؛ typecheck+build تمیز؛ هارنس محلی سبز؛ **دو اجرای پشت‌سرهم dev سبز**
  (۲۳ pass / ۲ skip / ۰ flaky در هر دو).

### ⛔ چه چیزی تحویل **نشد** (بانک‌شده، به ترتیبِ خودِ دستور)
ماژول امروز **فقط backend** است و از داخل برنامه دسترس‌پذیر نیست؛ به همین دلیل
پرچم روی همه خاموش است و ریسکِ زنده ندارد.
- **پای انبارِ برگشت‌ها** — `collect_events` امروز خرید، فروشِ نهایی‌شده و
  رویدادهای دستی را می‌خواند؛ **برگشت از فروش/خرید هنوز موجودی را حرکت نمی‌دهد.**
  این صریح‌ترین شکافِ باقی‌مانده نسبت به دستور است.
- API و UI کاردکس؛ کارت «موجودی» صفحهٔ کالا؛ انتخاب انبار؛ شبکهٔ ردیف فاکتور.
- گزارش‌ها: کاردکس چاپی، گردش موجودی‌ها، سفارش مجدد (حداقل/حداکثر)، dead stock.
  ستون‌های حداقل/حداکثر در دیتابیس هستند؛ گزارشش نیست.
- SKU «انبار و کاردکس» و اتصالش به pricing-v2.
- eval روی «ترازپیشه دیبا» (موجودی اول دوره، انبار دوم، انتقال، فروشِ منفی).
- راهنماهای بازرگان/حسابدار و اسپکِ هارنسِ انبار.



## 2026-08-06 — گیت استقرار تعمیر شد + «مشتریان بی‌خرید» + نقشهٔ گزارش‌ها

SHAs: backend **`e1a4aa6`** · frontend **`5971d54`** · ops **`ef7f193`**.
مهاجرت جدید ندارد (head همچنان `pay1405a006`). مستقر روی dev.

**اسپک ۱۸ — گیت استقرار دیگر دروغ نمی‌گوید (ریشه پیدا و بسته شد).**
علتِ واقعی این نبود که «reset فقط وقتی کسی فعال نبود اجرا می‌شد»؛ این بود که اسپک
هدفش را از فهرستِ **پیش از** reset برمی‌داشت ولی در UI روی **اولین** دکمهٔ
«تسویه‌حساب» کلیک می‌کرد. و چون `void_settlement` عمداً کارمند را **دوباره فعال
می‌کند** (تسویه پس گرفته شده، پس اخراج هم پس گرفته می‌شود)، بعد از reset ترتیبِ
کارت‌ها عوض می‌شد: یک نفر تسویه می‌شد و دربارهٔ نفرِ دیگری assert می‌شد، و
`after.settlement` برابر null می‌آمد. dev به ۱۳ تسویه روی علی فخارمنش و **صفر**
روی محمد هدایت رسیده بود.
- **تعمیر دنیای dev از مسیر استاندارد:** تسویهٔ `paid`ِ باقی‌مانده void شد — که
  خودش علی را دوباره فعال کرد. حالا هر دو کارمند فعال‌اند و هیچ تسویهٔ زنده‌ای
  نیست. (`is_active` عمداً PATCH‌پذیر نیست؛ void تنها راهِ صادقانهٔ برگشت است.)
- **اسپک بی‌قید و شرط خودبازگردان شد:** همهٔ تسویه‌های زندهٔ همهٔ کارکنان void
  می‌شوند (حلقه‌ای، چون API هر بار یک تسویهٔ زنده می‌دهد)، فهرست **بعد از** reset
  دوباره خوانده می‌شود، و کلیک داخل کارتِ خودِ هدف از راه `data-employee-id`
  انجام می‌شود — نه روی «اولین دکمهٔ روی صفحه».
- **گیت خودش: دو اجرای پشت‌سرهم روی dev، هر دو سبز** (exit 0):
  اجرای ۱ → ۲۲ pass / ۱ flaky / ۲ skip؛ اجرای ۲ → ۲۳ pass / ۱ flaky / ۲ skip.
  اسپک ۱۸ در هر دو سبز.
- **حکمِ flake اسپک ۰۳ (هلدینگ): بازتولید نشد** — در هر دو اجرا بارِ اول سبز شد.
- ⚠️ **flake تازه و تکرارشونده:** `archive-and-delete-protection` در **هر دو**
  اجرا بارِ اول قرمز و در retry سبز شد. تشخیص: اسپک متنِ **کلِ صفحه** را مقایسه
  می‌کند (`profitText`) و اسنپ‌شاتِ دوم را پیش از رندرشدنِ دوبارهٔ جدولِ فاکتورها
  می‌گیرد (رشتهٔ Received جدول ندارد). گیت به‌خاطر retry سبز است، ولی باید روی
  یک عددِ مشخص assert کند نه روی متنِ کل صفحه. **پیگیری باز.**
- یادداشت صادقانه: هر اجرا یک تسویهٔ **voided** به تاریخچه اضافه می‌کند (علی الان
  ۱۵ ردیف: ۱ paid + ۱۴ voided). این درست است — void حذف نیست، ردِ حسابرسی است.

**«مشتریانی که مدتی است خرید نکرده‌اند» (`GET /reports/dormant-customers`)**
مشتریانی که در بازه هیچ فاکتور **نهایی‌شده‌ای** ندارند ولی **پیش از** آن داشته‌اند،
به ترتیبِ قدیمی‌ترین خریدِ آخر. شرطِ «قبلاً خرید کرده» لازم است نه تزئین: بدونش هر
مشتریِ تازه‌ثبت‌شده هم در فهرست می‌آمد و گزارش از روز اول بی‌مصرف می‌شد.
- بازه‌های آماده: ۳۰ روز / ۹۰ روز / از ابتدای سال مالی / بازهٔ دلخواه شمسی.
  «از ابتدای سال مالی» **تا امروز** است نه تا پایان سال — بازه‌ای که به آینده برود
  هر مشتری را فعال نشان می‌دهد.
- ستون‌ها: نام، آخرین خرید (شمسی + «چند وقت پیش»)، جمع و تعداد خریدِ قبلی، ماندهٔ
  فعلی؛ پرش به پروندهٔ مشتری؛ جستجوی نام؛ خروجی Excel/PDF/CSV؛ حالت خالیِ آرام.
- **نکتهٔ فنی:** `correlate(Customer)` روی EXISTS لازم است، وگرنه auto-correlation
  هر دو جدول را بیرون می‌کشد و پرس‌وجو با «no FROM clauses» ۵۰۰ می‌دهد. ۴ آزمون
  pg واقعی (پیش‌نویس‌ها در هر دو جهت نامرئی‌اند، ترتیب، جستجو، حالت خالی).
- **اثبات تجربی روی دیباتک:** خروجی API با یک پرس‌وجوی مستقل SQL مو‌به‌مو یکی شد
  (رایان‌پرداز ۲ فاکتور / ۲۱۴٬۵۰۰٬۰۰۰ / آخرین ۱۴۰۴/۱۲/۱۲؛ ابری پارس ۲ /
  ۱۵۴٬۰۰۰٬۰۰۰ / ۱۴۰۴/۱۲/۱۳؛ صفر فاکتور در بازه).
- ۳۹۰ پیکسل: چهار ستون جا نمی‌شد و هر دو ستون مبلغ پشت اسکرول افقی می‌افتاد؛
  «جمع خرید قبلی» روی موبایل زیر نام مشتری می‌نشیند.

**نقشهٔ گزارش‌ها** — `docs/report_roadmap.md` (تازه) + بند ۶ در
`inventory_reference_notes.md`: فلسفهٔ «حدود ده گزارشِ منتخب و فیلترپذیر، نه جنگلِ
گزارش»، برداشتِ بستهٔ انبار (کاردکسِ واحد دوم، کنترل موجودی با حداقل/حداکثر ⇒
هشدار سفارش مجدد، dead stock، گردش به تفکیک انبار، فرم‌های چاپی)، نامزدهای پس از
لانچ، و نه‌گفتن‌های صریح (گزارش‌سازِ پارامتریک، سهامِ تعاونی، چندارزی).


## 2026-08-05 — چاپ حرفه‌ای سند + بستن سه سؤال حسابدار + بانک مرجع

SHAs: backend **`a7af78d`** · frontend **`c2260f7`** · ops **`e731926`**.
مهاجرت جدید ندارد (head همچنان `pay1405a006`). مستقر روی dev.

**سند حسابداری، به استاندارد دفترداری (`voucher.py` + `entry-view.tsx`)**
- ساختار **کل → معین → تفصیلی** با جمعِ رول‌آپ روی هر سطح؛ شرح و مبلغ جزء روی
  ردیف تفصیلی، مبلغ بستانکار داخل پرانتز (قرارداد خودِ دفترداری).
- سربرگ معین وقتی فقط خودش را تکرار کند حذف می‌شود — در چارت ما فروش/خرید مستقیم
  روی معین ثبت می‌شوند، و بدون این قاعده هر سند یک سطر تکراری می‌گرفت.
- سطر **جمع** روی ردیف‌های **تفصیلی** بسته می‌شود. مرجع (سند ۳۲۹) سطرهای کل و معین
  را هم می‌شمارد و هر مبلغ را دوبار حساب می‌کند (۱۲٬۰۱۸٬۴۳۶٬۴۵۴ = ۲× واقعیِ
  ۶٬۰۰۹٬۲۱۸٬۲۲۷)؛ آن را عمداً نگرفتیم.
- سه جای امضا (تهیه/تأیید/تصویب) + خروجی **PDF** تازه:
  `GET /accounting/journal/{id}/voucher.pdf` — A4 راست‌به‌چپ، شمارهٔ صفحهٔ
  **فارسی** `counter(page, persian)`، سربرگ کسب‌وکار، RTL تمیز (با `pdftotext`
  تأیید شد).
- **۳۹۰ پیکسل:** هفت ستون جا نمی‌شد — شرح در شش خط می‌شکست و سطر «جمع» پشت اسکرول
  افقی از صفحه بیرون می‌افتاد. موبایل به چهار ستون می‌رسد (ردیف · کد · نام حساب ·
  مبلغ) و رول‌آپ در ستون مبلغ می‌نشیند؛ چاپ و PDF همهٔ ستون‌ها را برمی‌گردانند.
- `groups` فقط روی endpoint تک‌سند پر می‌شود؛ فهرست روزنامه هزینهٔ خواندن درخت
  حساب‌ها را نمی‌دهد.

**ممیزی شرحِ ردیف (تجربی، روی دفتر واقعی):** هر ۱۸ نوع منبع شرحِ **سند** را پر
می‌کنند (۰ خالی، حداقل ۱۰ نویسه)؛ ولی فقط **۱۲ از ۴٬۱۷۱** ردیف شرحِ خودشان را
دارند — یعنی صرفاً اسناد دستی. هر ۸۲ جای ساختِ `_Line` در `journal.py` بدون
`description` می‌سازد. چاپ همان کاری را می‌کند که مرجع می‌کند: شرح ردیف اگر بود،
وگرنه شرح سند (`line_narration`). **پیگیری باز:** ماندگارکردن شرحِ ردیف در
مولدها — نیازمند regenerate کامل برای پرکردن گذشته، و دست‌زدن به ۸۲ closure در
موتور سند تراز؛ عمداً به این بسته اضافه نشد.

**نسبت‌های مالی — سه سؤال بسته شد**
- «نسبت کل دارایی به کل بدهی‌ها» و «نسبت بدهی به فروش»: حسابدار فرمول استاندارد
  ما را تأیید کرد (۱۴۰۵/۰۵/۱۵)؛ یادداشت کهربایی هر دو کارت **برداشته شد**.
- «ارزش ویژه» **باگ داشت**: کارت `value=None` می‌فرستاد «چون نسبت نیست»، و صفحه
  هر `value` تهی را «مخرج این نسبت صفر است» می‌خواند — پس یک حقوق صاحبان سرمایهٔ
  کاملاً درست‌محاسبه‌شده پشت یک خطای دروغین پنهان می‌شد. حالا `display="amount"`
  است، مبلغ را با سه جزء (سرمایه + اندوختهٔ قانونی + سود/زیان انباشته) نشان
  می‌دهد، و `pending` آرام (خاکستری، نه کهربایی) می‌گوید فرم نسبتی **در انتظار
  فرمول مکتوب حسابدار** است. هیچ نسبتی حدس زده نشد.
- فهرست کوتاهِ باقی‌مانده برای حسابدار: `docs/financial_ratios_mapping.md` بند ۳٫۱
  (۴ سؤال قدیمی + فرمول نسبتیِ «ارزش ویژه»).

**بانک مرجع (فقط مستندات)** — `docs/inventory_reference_notes.md` و
`docs/invoice_print_gap_audit.md` از چهار خروجی چاپیِ دفاتر یک شرکت واقعی. لنگرِ
آزمونِ بستهٔ انبار: سند فروشِ مرکب (۴٬۲۱۳ = ۳٬۸۳۰ + ۳۸۳ به‌علاوهٔ پای بهای
تمام‌شده/موجودی در همان سند) و میانگین موزون (۲@۵۱ + ۱@۳۹ ← ۳ @ ۴۷). خودِ PDFها
در `.gitignore` رفتند — شناسهٔ ملی و کد اقتصادی یک شرکت واقعی رویشان است.

**بدهی بصری بسته‌شده:** زنجیرهٔ چکِ راه‌رفته در «اسناد در جریان وصول ۱۳۰۲» روی
دفتر حساب (واگذاری ← وصول، ماندهٔ پایان صفر) و برچسب «نرخ توافقی» بهرام سقفی
کنار دو همکارِ نرخ‌فرمولی، هر دو در ۳۹۰ و دسکتاپ.

> ⚠️ **هارنس روی dev — اسپک ۱۸ (تسویه‌حساب) بارِ دوم قرمز می‌شود.** اجرای اولِ
> کاملِ هارنس روی dev **سبز** بود (۲۳ pass، اسپک ۱۸ اعداد واقعی چاپ کرد:
> severance ۷۳۴٬۵۲۰٬۵۴۷). اجرای دومِ پشت‌سرهم: **۲۲ pass / ۱ fail / ۲ skip** — و
> شکست فقط همین اسپک است. ربطی به این بسته ندارد (حقوق/تسویه دست نخورده).
> علتِ تجربی از psql روی dev: علی فخارمنش الان **۹** تسویه‌حساب دارد (۸ voided +
> **۱ paid**) و محمد هدایت هیچ — یعنی «self-reset»ِ خودِ اسپک دنیا را کامل
> برنمی‌گرداند: settlementِ `paid`ِ قدیمی باقی می‌ماند و کارمند inactive می‌ماند،
> پس اجرای بعدی هدفِ سالمی برای walk ندارد. **پیگیری:** اسپک ۱۸ باید idempotent
> شود (voidِ همهٔ تسویه‌ها نه فقط آخری + برگرداندنِ is_active)، وگرنه گیتِ استقرار
> به دلیلِ غلط قرمز می‌شود — و گیتی که بی‌دلیل قرمز شود، یاد می‌دهد نادیده‌اش
> بگیرند. اسپک ۰۳ (هلدینگ) هم یک‌بار fail و در retry سبز شد (flake).

> ⚠️ **نکتهٔ دادهٔ seed (نه باگ کد):** در دفتر ۱۳۰۲ روی dev، «وصول چک DIBA-R660»
> با تاریخ ۲۲ اردیبهشت **پیش از** «واگذاری» همان چک با تاریخ ۱۳ مرداد می‌نشیند، و
> ماندهٔ در جریان موقتاً منفی می‌شود. چکِ واقعی نمی‌تواند پیش از واگذاری وصول شود
> — تاریخ‌های دنیای نمونه باید مرتب شوند.
