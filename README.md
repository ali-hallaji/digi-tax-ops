...
| `09120001006` | `Admin@12345` | member_admin | پخش آریا (مدیر) | — | دسترسی مدیر: همهٔ ۲۰ فاکتور پخش آریا را می‌بیند |
| `09120001007` | `Admin@12345` | member_staff | پخش آریا (کارمند) | — | دسترسی کارمند: فقط ۷ فاکتور خودش را می‌بیند (own-only) |
| `09120001004` | `Admin@12345` | partner_approved | — | حقیقی | پنل همکار (داخل /admin): مشتریان من (۵ کسب‌وکار)، درآمد و تسویه، کد همکار HAM-TEST1 |
| `09120001005` | `Admin@12345` | partner_pending | — | حقیقی | وضعیت «در حال بررسی» درخواست همکاری |
...
| `09300000000` | فقط کد یکبارمصرف | user (بدون کسب‌وکار) | — | — | سفر اولین‌بار از صفر: ورود → ویزارد ساخت کسب‌وکار → داشبورد فعال‌سازی — هیچ داده‌ای از قبل نیست |
| `09300000001` | فقط کد یکبارمصرف | user (بدون کسب‌وکار) | — | — | سفر اولین‌بار از صفر — نسخهٔ دستْ‌نخوردهٔ دوم، برای وقتی که خام اول مصرف شده |
<!-- PERSONA-LOGINS:END -->

> ⚠️ **انحرافِ زمانِ‌اجرا روی dev — «کاربر خام» و «کاربر خام دوم» (۱۴۰۵/۰۵/۱۱).**
> این یادداشت عمداً **بیرون از بلوک تولیدشده** است تا با بازتولید جدول پاک نشود.
> جدول بالا قرارداد **seed** را می‌گوید؛ این یادداشت می‌گوید روی dev **چه چیزی فرق دارد**.
>
> - `09300000001` («کاربر خام دوم») روی dev **رمز دارد** — یک حسابدارِ آزمایش‌کننده
>   خودش رمزش را عوض کرد (**تأییدشدهٔ بنیان‌گذار**). در `world_fixtures.py` مقدار
>   `password: None` است، پس این انحراف **درست و مورد انتظار** است.
> - `09300000000` («کاربر خام») هم روی dev رمز دارد (مقدارش نه `Admin@12345` است و نه
>   رمز بالا) — یعنی آن هم دست‌خورده است.
> - **رمزهای انتخابیِ این دو کاربر عمداً اینجا ثبت نشده‌اند**: رمزی که یک انسان واقعی
>   برای خودش انتخاب کرده رمز عبور واقعی است، نه دادهٔ دمو، و در git نوشته نمی‌شود.
>   برای یافتنش با `verify_password` روی dev پروب بزن، نه با حدس.
>
> **کاری که نباید کرد:** دیدنِ ۴۰۱ روی این دو شماره «باگ» نیست و دلیل reseed نیست
> (درسِ زهرا محمدی). با **کد یکبارمصرف** وارد شو — که برای هر دو هنوز کار می‌کند.
>
> **هشدار جداگانه و مهم‌تر:** `09300000001` اکنون **۱ کسب‌وکار دارد**، یعنی دیگر
> «دستْ‌نخورده» نیست و کل هدفش را از دست داده. `09300000000` هنوز صفر کسب‌وکار دارد
> ولی رمزش عوض شده. **در حال حاضر روی dev هیچ پرسونای کاملاً بکری برای سفرِ
> اولین‌بار باقی نمانده** — اگر به یکی نیاز داری، از بنیان‌گذار برای افزودنِ
> پرسونای خام سوم (append، طبق §۴.۶) اجازه بگیر؛ این دو را reseed نکن.
>
> **یادداشت سازگاری اسناد:** متنِ §۴.۶ در `CLAUDE.md` هنوز می‌گوید `09120001005` و
> `09120001006` و `09120001007` «فقط dev-OTP» هستند. این **کهنه** است: منبع یگانهٔ
> حقیقت (`world_fixtures.py`) به همهٔ پرسوناها جز دو کاربر خام `SEED_PASSWORD` می‌دهد،
> و dev هم دقیقاً همین است. جدول بالا درست است، نه آن جمله.
